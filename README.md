...
- `/魔法少女黑化`：与日常日记共用存档、队友识别、关系总结和保存流程，但使用独立的黑化 Prompt。命令后可写本次黑化行动；不写行动时由 LLM 根据当前状态自由生成。
- `/反派干部战斗`：与日常日记共用存档、队友识别、关系总结和保存流程，但使用独立的反派干部战斗 Prompt。命令后可写本次战斗行动；不写行动时由 LLM 根据当前状态自由生成。
